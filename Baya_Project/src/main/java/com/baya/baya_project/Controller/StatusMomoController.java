...
@WebServlet(name = "StatusMomo", value = "/status-momo")
public class StatusMomoController extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if(status.equalsIgnoreCase("0")){
            session.setAttribute("method", "momo");
            OrderService service = new OrderService();
            Order order =service.insertOrder((User) session.getAttribute("user"), (Cart)session.getAttribute("cart"),name,email,phone,address,note,"momo");
